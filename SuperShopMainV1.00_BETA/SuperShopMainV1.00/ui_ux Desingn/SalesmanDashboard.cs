...
namespace SuperShopMainV1._00
{
    public partial class SalesmanDashboar : Form
    {
        ...
        public void effectprod()
        {
            ...
            {
                string query = "Select SOLD from PRODUCT WHERE PRODUCT_ID =" + int.Parse(customersalesdatagrid.Rows[i].Cells[0].Value.ToString());
                SqlDataAdapter sda = new SqlDataAdapter(query, sqlcon);
                DataTable dtb = new DataTable();
                sda.Fill(dtb);
                arr[i] = dtb.Rows[0][0].ToString();
               
            }
            for (int i = 0; i < customersalesdatagrid.Rows.Count; i++)
            {

                prr[i] = int.Parse(arr[i]) + int.Parse(customersalesdatagrid.Rows[i].Cells[2].Value.ToString());
            ...
            for (int i = 0; i < customersalesdatagrid.Rows.Count; i++)
            {
                ...
                SqlCommand cmd = new SqlCommand("UPDATE PRODUCT SET SOLD=@SOLD Where PRODUCT_ID=@PRODUCT_ID", sqlcon);
                cmd.Parameters.AddWithValue("@SOLD",prr[i]);
        ...
        private void proceedtopay_Click(object sender, EventArgs e)
        {
            ...
            {
                if (memberidcheckbox.Text != ""&&paymethondselector.Text!=""&&CashRecievetextbox.Text!="")
                {
                    if (totalcounter <= double.Parse(CashRecievetextbox.Text))
                    {
                        try
                        {
                            sqlcon.Open();
                        ...
                        MessageBox.Show("Recieved Amount is less then Total Payable");
                    }

                }
                if (memberidcheckbox.Text == "")
                {
                    MessageBox.Show("You have seleceted Member but not enter member_ID");
                    if (paymethondselector.Text == "")
                    {
            ...
            if(memberdropdown.Text=="Non Member")
            {
                if (memberidcheckbox.Text != "")
                {
                    MessageBox.Show("You have selected Non-Member status but Member_ID is not empty");
                }
                if (memberidcheckbox.Text == "" && paymethondselector.Text != ""&&CashRecievetextbox.Text!="")
                {
                    if (totalcounter <= double.Parse(CashRecievetextbox.Text))
